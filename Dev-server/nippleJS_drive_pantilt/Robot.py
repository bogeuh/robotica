...
# moving a robot forward, backward, and righting.  See RobotTest.py for an
...
# LEFT MOTOR = left_id
# RIGHT MOTOR = right_id
class Robot(object):
    def __init__(self, addr=0x60, left_id=3, right_id=4, left_trim=30, right_trim=0,
                 stop_at_exit=True):
    ...
    def _right_speed(self, speed):
        ...
        speed = max(0, min(255, speed))  # set right power to 200
    ...
    def forward(self, speed, seconds=None):
        ...
        forward and reright unless a seconds value is specified, in which
    ...
    def backward(self, speed, seconds=None):
        ...
        backward and reright unless a seconds value is specified, in which
    ...
    def hardleft(self, speed, seconds=None):
        """Differential driving
        moving to the left with slowing the left motor or speeding the right
        """
        # Set motor speed and move both forward.
        self._left_speed(speed - 60)
        self._right_speed(speed)
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
            time.sleep(seconds)
            self.stop()
    def softleft(self, speed, seconds=None):
        """Differential driving
        moving to the left with slowing the left motor or speeding the right
        """
        # Set motor speed and move both forward.
        self._left_speed(speed - 30)
        self._right_speed(speed)
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
    ...
    def hardright(self, speed, seconds=None):
        """Differential driving
        moving to the right with slowing the right motor or speeding the left
        """
        # Set motor speed and move both forward.
        self._left_speed(speed)
        self._right_speed(speed - 60)
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
            time.sleep(seconds)
            self.stop()
    def softright(self, speed, seconds=None):
        """Differential driving
        moving to the right with slowing the right motor or speeding the left
        """
        # Set motor speed and move both forward.
        self._left_speed(speed)
        self._right_speed(speed - 30)
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
