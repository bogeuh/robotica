...
def control_event():
    #make global so pan and tilt remember where they are
    global tilt_angle
    global pan_angle

    if request.method == 'POST':
        control = request.form['control']
        # ptdir = request.form['ptdir']
        # ptangle = request.form['ptangle']
        if control == 'f':
            try:
                robot.forward(150)
            except:
           ...
           # global tilt_angle
            ...
            #pantilthat.servo_enable(1, 1)
            #pantilthat.servo_enable(2, 1)
...
def api(direction, angle):
    if angle < 0 or angle > 120:
        #should never happen unless javascript is bugged :)
        return "{'error':'out of range'}"

    #use this to get angle from -60 to +60 instead of 0 to 120
    angle -= 60
